...
async function createPinguPagTransaction({ amount, name, cpf, phone, email, title }) {
  ...
  const payload = {
    ...
    source: 'api_externa', // Allows external transactions without pre-registered productHash
    customer: {
      name: name || 'Beneficiário Gov',
      email: cleanEmail,
      phone: cleanPhone,
...
function getGatewayConfig() {
  ...
  const gatewaysList = Object.keys(db.gateways).map(k => {
    ...
    const reqPub = k === 'freepay'; // Only FreePay needs public key; PinguPag, BlackCat, FlevoPay use secretKey only
    return {
      key: gw.key,
      name: gw.name,
      label: gw.label,
