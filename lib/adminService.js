...
function getInitialDB() {
  return {
    config: {
      ...
      adminPassword: process.env.ADMIN_PASSWORD || 'admin@recupera2026'
    },
...
function loginAdmin(username, password) {
  const db = loadDB();
  const validUser = db.config.adminUser || 'admin';
  const validPass = db.config.adminPassword || 'admin@recupera2026';
