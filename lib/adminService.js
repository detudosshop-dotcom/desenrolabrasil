...
async function createFreePayTransactionTest(publicKey, secretKey) {
  ...
  const testPayload = {
    ...
    items: [
      {
        title: 'gateway-test-freepay-' + Date.now(),
        unit_price: 100,
        quantity: 1,
        tangible: false
      }
...
async function createFlevoPayTransaction({ amount, name, cpf, phone, email, title }) {
  ...
  const payload = {
    amount: amountCents,
    description: title || 'Quitação de Dívidas - Programa Desenrola Brasil',
...
async function createPinguPagTransaction({ amount, name, cpf, phone, email, title }) {
  ...
  if (!apiKey) {
    throw new Error('Chave de API (Secret Key / API Key) da PinguPag não configurada.');
  }
  ...
  const res = await fetch(`${PINGU_BASE_URL}/gateway/v1/transactions`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-API-Key': apiKey
  ...
  const data = await res.json();
  if (!res.ok || (data.status && data.status !== 'success' && !data.qr_code)) {
    const errorMsg = data.message || data.error || `Falha na API PinguPag (HTTP ${res.status})`;
    throw new Error(errorMsg);
  }
...
function getGatewayConfig() {
  ...
  const gatewaysList = Object.keys(db.gateways).map(k => {
    ...
    return {
      ...
      isConfigured: Boolean(gw.publicKey && gw.secretKey)
    };
  });
...
function updateGatewayConfig({ gatewayKey, secretKey, publicKey, maxAmountCents }) {
  const db = loadDB();
  ...
  saveDB();
  return { success: true, message: 'Configurações de gateway salvas com sucesso!' };
}
