...
// Default gateways with FreePay and BlackCat adapters enabled
  ...
  if (targetKey === 'freepay') {
    try {
      ...
      if (testRes && testRes.success && testRes.data) {
        ...
        const errMsg = testRes?.message || testRes?.error || 'Não foi possível gerar o PIX de teste de R$ 1,00 na FreePay.';
        return {
          success: false,
          error: `Falha no teste da gateway FreePay: ${errMsg}. A gateway anterior foi mantida ativa.`
        };
      }
    } catch (err) {
      return {
        success: false,
        error: `Erro na comunicação com a API FreePay: ${err.message}. A gateway anterior foi mantida ativa.`
      };
    }
  }
