...
function getUtmifyApiTokens() {
  const db = loadDB();
  if (Array.isArray(db.config?.utmifyApiTokens) && db.config.utmifyApiTokens.length > 0) {
  ...
  return [
    '487lh6GrQqPa4qvYSNBIO4a7tnBxtZNKoYIe',
    'FVCewCTST4jON2YWRwue5Cl0o4xzJETGKsOA'
  ];
}
...
function updateUtmifyApiTokens(tokens) {
  ...
  db.config.utmifyApiTokens = list.length > 0 ? list : ['487lh6GrQqPa4qvYSNBIO4a7tnBxtZNKoYIe'];
  db.config.utmifyApiToken = db.config.utmifyApiTokens[0] || '';
  saveDB();
