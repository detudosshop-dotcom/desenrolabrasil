...
const DEFAULT_GATEWAYS = {
  ...
  flevopay: {
    ...
    state: 'Configurado',
    statusText: 'Configurado e disponível',
    publicKey: process.env.FLEVO_PUBLIC_KEY || '',
...
function loadDB() {
  ...
  if (fs.existsSync(DB_FILE)) {
    try {
      ...
      if (!dbMemory.config) dbMemory.config = getInitialDB().config;
      if (!dbMemory.gateways) dbMemory.gateways = DEFAULT_GATEWAYS;
      if (!dbMemory.offers) dbMemory.offers = DEFAULT_OFFERS;
      if (!dbMemory.orders) dbMemory.orders = [];
      if (!dbMemory.sessions) dbMemory.sessions = DEFAULT_SESSIONS;
      if (!dbMemory.authSessions) dbMemory.authSessions = {};

      // Ensure FlevoPay and PinguPag adapters are marked true
      if (dbMemory.gateways.flevopay) {
        dbMemory.gateways.flevopay.hasAdapter = true;
        dbMemory.gateways.flevopay.statusText = 'Configurado e disponível';
      }
      if (dbMemory.gateways.pingupag) {
        dbMemory.gateways.pingupag.hasAdapter = true;
        dbMemory.gateways.pingupag.statusText = 'Configurado e disponível';
      }
      if (dbMemory.gateways.blackcat) {
        dbMemory.gateways.blackcat.hasAdapter = true;
        dbMemory.gateways.blackcat.statusText = 'Configurado e disponível';
      }

      return dbMemory;
    } catch(err) {
      console.error('Error reading db.json, resetting to default:', err.message);
    }
  }
...
function loginAdmin(username, password) {
  ...
  if (username === validUser && password === validPass) {
    const token = 'recupera_sess_' + crypto.randomBytes(32).toString('hex');
...
async function createFlevoPayTransaction({ amount, name, cpf, phone, email, title }) {
  const apiKey = getFlevoApiKey();
  if (!apiKey) {
    throw new Error('Chave de API (Secret Key / API Key) da FlevoPay não configurada.');
  }
  ...
  const res = await fetch(`${FLEVO_BASE_URL}/api/v1/transactions`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-API-Key': apiKey
  ...
  const data = await res.json();
  if (!res.ok || (data.status && data.status !== 'success' && !data.qr_code)) {
...
function getGatewayConfig() {
  ...
  const gatewaysList = Object.keys(db.gateways).map(k => {
    ...
    const reqPub = k !== 'pingupag'; // Only PinguPag does not use public key; all others request both secret and public keys
    return {
      key: gw.key,
      name: gw.name,
      label: gw.label,
...
function updateGatewayConfig({ gatewayKey, secretKey, publicKey, maxAmountCents, setActive }) {
  const db = loadDB();
  const targetKey = gatewayKey || db.config.activeGateway || 'freepay';
...
const DEFAULT_PIXELS = {
  utmify: [
    { id: 'utm_1', name: 'Token UTMify Principal', token: 'utm_live_9a8b7c6d5e4f3a2b1c', active: true }
  ],
  facebook: [
    { id: 'fb_1', name: 'Pixel Meta Principal', pixelId: '1392443726166280', code: '', token: '', active: true }
  ],
  tiktok: [
    { id: 'tt_1', name: 'Pixel TikTok Principal', pixelId: 'DA9IHQBC77UBPDTVJ18G', code: '', token: '', active: true }
  ]
};
...
function savePixels({ utmify, facebook, tiktok }) {
  const db = loadDB();
  db.pixels = {
    utmify: Array.isArray(utmify) ? utmify.map(u => ({
      id: u.id || 'utm_' + Date.now() + '_' + Math.random().toString(36).substr(2, 5),
      name: (u.name || '').trim(),
      token: (u.token || '').trim(),
      active: u.active !== undefined ? Boolean(u.active) : true
    })) : [],
    facebook: Array.isArray(facebook) ? facebook.map(f => ({
    ...
    tiktok: Array.isArray(tiktok) ? tiktok.map(t => ({
  ...
  saveDB();
  return { success: true, message: 'Todos os pixels foram salvos e aplicados com sucesso!' };
}
...
// ----------------------------------------------------
// UTMIFY API & WEBHOOKS DISPATCHER
// ----------------------------------------------------
function getUtmifyApiToken() {
  const db = loadDB();
  return db.config?.utmifyApiToken || process.env.UTMIFY_API_TOKEN || '487lh6GrQqPa4qvYSNBIO4a7tnBxtZNKoYIe';
}

function updateUtmifyApiToken(token) {
  const db = loadDB();
  db.config.utmifyApiToken = (token || '').trim();
  saveDB();
  return { success: true, message: 'Token da API Utmify salvo com sucesso!' };
}
...
async function sendUtmifyOrderWebhook({
  ...
  if (!token) {
    console.log('[Utmify] Token não configurado, ignorando webhook.');
    return { success: false, error: 'Token da Utmify não configurado' };
  }
  ...
  try {
    ...
    console.log(`[Utmify Webhook] Order: ${payload.orderId} | Status: ${status} | HTTP: ${res.status} | Result:`, resData);
    return {
      success: res.ok && resData.OK !== false,
      statusCode: res.status,
      data: resData
    };
  } catch (err) {
    console.error('[Utmify Webhook Error]:', err.message);
    return { success: false, error: err.message };
  }
}
