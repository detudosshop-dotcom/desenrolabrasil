...
const DEFAULT_GATEWAYS = {
  ...
  pingupag: {
    ...
    secretKey: process.env.PINGU_API_KEY || process.env.PINGU_SECRET_KEY || 'sk_363807e79eeef7a59f5f357a7782fd85f0bbbe8c05e0372fc07a53f03ff9e1d5',
    maxAmountCents: 100000
  },
  duttyfy: {
    key: 'duttyfy',
...
function loadDB() {
  ...
  if (!dbMemory.pixels) dbMemory.pixels = { utmify: [], facebook: [], tiktok: [] };
...
async function createUniversalTransaction({ amount, name, cpf, phone, email, title }) {
  const db = loadDB();
  const activeKey = db.config?.activeGateway || 'pingupag';
  ...
  if (activeKey === 'freepay') {
    try {
      const pub = decodeKeyIfNeeded(gw.publicKey) || process.env.FREEPAY_PUBLIC_KEY || 'freepay_live_qpK0a9csPUsK8gIN2cFbl23sEEFWJRW3';
      const sec = decodeKeyIfNeeded(gw.secretKey) || process.env.FREEPAY_SECRET_KEY || decodeKeyIfNeeded('B64:c2tfbGl2ZV9tSGkxM3g3aTdyNnk0c2I2YUR5OFduMURWQWUxZGF4cw==');
